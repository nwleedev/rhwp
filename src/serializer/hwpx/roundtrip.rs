...
fn diff_paragraph_char_shapes(
    ...
    let sa: Vec<&Control> = pa
        .controls
        .iter()
        .filter(|c| is_roundtrip_control_slot(c))
        .collect();
...
fn is_roundtrip_control_slot(control: &crate::model::control::Control) -> bool {
    use crate::model::control::Control::*;
    is_hwpx_inline_slot(control) && !matches!(control, ColumnDef(_) | Bookmark(_) | Markpen(_))
}
