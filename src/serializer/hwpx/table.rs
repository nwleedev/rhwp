...
use super::section::{render_hp_p_open_allocated, render_paragraph_parts};
...
fn write_sub_list_paragraphs<W: Write>(
    ...
    for para in paragraphs {
        ...
        vert_cursor = advance;
        let mut p_xml = render_hp_p_open_allocated(para, ctx);
        p_xml.push_str(&runs);
...
mod tests {
    ...
    fn table_caption_serializes_sublist_before_in_margin() {
        ...
        assert!(xml.contains(
            r#"<hp:caption side="TOP" fullSz="1" width="3000" gap="120" lastWidth="4000">"#
        ));
        assert_eq!(
    ...
    #[test]
    fn table_pos_preserves_flow_with_text_false() {
        let mut t = empty_table(1, 1);
        ...
        assert!(
            ...
            "table pos must preserve source flowWithText=0: {}",
            xml
        );
    }
    ...
    fn task1379_cell_char_overlap_emitted_as_compose() {
        ...
        assert!(
            xml.contains(r#"<hp:charPr prIDRef="37"/><hp:charPr prIDRef="-1"/></hp:compose>"#),
            "charPr 목록이 HWPX 음수 sentinel(-1)로 방출되어야 함: {}",
            xml
        );
    }
